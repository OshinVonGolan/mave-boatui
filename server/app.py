...
from server.ki import Ki, KiFehler
...
# ── KI im Logbuch ───────────────────────────────────────────────────────────
# EIN Zugang fuer alle Funktionen, die spaeter dazukommen — ein Bericht aus
# dem Mitschnitt, eine Erklaerung zu einem Alarm. Sie rufen `ki.frage()`.
#
# Angemeldet wird mit dem Abo des Eigners. Deshalb liegt der Zugang hinter
# `einstellen` und nicht hinter `lesen`: wer ihn hat, verbraucht sein
# Kontingent.

ki = Ki(DATEN / 'ki.json')


class KiCode(BaseModel):
    code: str


@app.get('/api/logbuch/ki')
def logbuch_ki(k: dict = Depends(braucht(r.EINSTELLEN))) -> JSONResponse:
    return JSONResponse(ki.zustand())


@app.post('/api/logbuch/ki/start')
def logbuch_ki_start(k: dict = Depends(braucht(r.EINSTELLEN))) -> JSONResponse:
    """Die Adresse, an der man sich bei Claude anmeldet.

    Von dort kommt ein Code zum Abtippen zurueck — es gibt keinen Rueckweg in
    diese Anwendung, und das ist gut so: der Server steht hinter Caddy und hat
    keine Adresse, die Anthropic kennen muesste.
    """
    return JSONResponse(ki.anmeldung_starten())


@app.post('/api/logbuch/ki/code')
async def logbuch_ki_code(body: KiCode,
                          k: dict = Depends(braucht(r.EINSTELLEN))) -> JSONResponse:
    try:
        await ki.anmeldung_abschliessen(body.code)
    except KiFehler as e:
        raise HTTPException(400, detail=str(e)) from None
    return JSONResponse({'ok': True, **ki.zustand()})


@app.delete('/api/logbuch/ki')
def logbuch_ki_weg(k: dict = Depends(braucht(r.EINSTELLEN))) -> JSONResponse:
    ki.abmelden()
    return JSONResponse({'ok': True, **ki.zustand()})
